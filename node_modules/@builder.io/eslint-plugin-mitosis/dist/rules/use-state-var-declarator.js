"use strict";
exports.__esModule = true;
var ts_pattern_1 = require("ts-pattern");
var isMitosisPath_1 = require("../helpers/isMitosisPath");
var noOp_1 = require("../helpers/noOp");
// ------------------------------------------------------------------------------
// Rule Definition
// ------------------------------------------------------------------------------
var rule = {
    meta: {
        type: 'problem',
        docs: {
            description: 'disallow assigning useState() to a variable with name other than state.',
            recommended: true
        }
    },
    create: function (context) {
        // variables should be defined here
        var filename = context.getFilename();
        if (!(0, isMitosisPath_1["default"])(filename))
            return {};
        // ----------------------------------------------------------------------
        // Helpers
        // ----------------------------------------------------------------------
        // any helper functions should go here or else delete this section
        // ----------------------------------------------------------------------
        // Public
        // ----------------------------------------------------------------------
        //
        var listener = {
            CallExpression: function (node) {
                (0, ts_pattern_1.match)(node)["with"]({
                    callee: {
                        name: 'useState'
                    },
                    parent: {
                        type: 'VariableDeclarator',
                        id: {
                            name: (0, ts_pattern_1.not)('state')
                        }
                    }
                }, function (node) {
                    context.report({
                        node: node.parent.id,
                        message: 'useState should be exclusively assigned to a variable called state'
                    });
                })
                    .otherwise(noOp_1["default"]);
            }
        };
        return listener;
    }
};
exports["default"] = rule;
